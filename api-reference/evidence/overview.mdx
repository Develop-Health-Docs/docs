...
Each content object also has an `other` key. You can use this nested object to include any other key value pairs that didn't fit into the predefined categories. This can be useful for adding other information that you think is useful that we haven't yet added to the top level fields. Our AI is able to handle a wide range of data and so we encourage you to send us as much relevant information as you can.
